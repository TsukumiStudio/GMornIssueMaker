...
func _run() -> void:
	...
	defaults.load_from_environment()
	assert(defaults.endpoint.is_empty() and defaults.repository.is_empty())
	ProjectSettings.set_setting("gmorn_issue_maker/labels", PackedStringArray(["test"]))
	defaults.load_from_environment()
	assert(defaults.labels == ["test"])
	...
	reporter.settings.reporter_id = "player-1"
	reporter.settings.reporter_name = "送信者"
	var game := {"残機": 3}
